...
## FLOW — ВИЗУАЛЬНЫЙ КОНСТРУКТОР AI-ПАЙПЛАЙНОВ [Документация](/docs/features/flow)

Flow позволяет создавать цепочки AI-генераций, соединяя блоки на визуальном канвасе.

### ТИПЫ БЛОКОВ:
- **Текст** — генерация текста через AI (Gemini). Используй для идей, промптов, описаний. Можно загрузить картинку/видео для анализа.
- **Изображение** — генерация картинок. Поддерживает: Nano Banana Pro, FLUX 2 Max/Pro, Ideogram V3, Recraft V3, Imagen 4.
- **Видео** — генерация видео. T2V модели (текст→видео) и I2V модели (картинка→видео).

### КАК РАБОТАЮТ СВЯЗИ:
- **Текст → Изображение**: текст из блока используется как промпт для картинки
- **Текст → Видео**: текст становится промптом для видео (Text-to-Video режим)
- **Текст → Текст**: текст передаётся как контекст для обработки AI (например, описание → промпт)
- **Изображение → Видео**: картинка анимируется (Image-to-Video режим). Формат видео = формат картинки!
- **Текст + Изображение → Видео**: картинка + текстовый промпт для описания движения
- **2 Изображения → Видео**: Keyframe режим — плавный морфинг от первой картинки ко второй

### РЕЖИМЫ ВИДЕО И ФОРМАТ (ASPECT RATIO):
- **T2V (Text-to-Video)**: только промпт, без картинки. Формат выбирается в настройках (только Kling, Veo).
- **I2V (Image-to-Video)**: картинка подключена. Формат видео = формат входной картинки. Настройка формата недоступна!
- **Keyframe**: 2 картинки подключены. Модель создаёт переход от первой ко второй. Поддерживают: Kling 2.5, Kling 2.1, Kling 1.0.

### КАКИЕ МОДЕЛИ ПОДДЕРЖИВАЮТ ВЫБОР ФОРМАТА:
- **Kling 2.5 Pro, Kling 2.1, Kling 2.0, Veo 3.1** — выбор формата (16:9, 9:16, 1:1) в T2V режиме
- **Hailuo, Seedance, Wan** — НЕ поддерживают выбор формата. Формат определяется автоматически или от картинки.

### КАК РАБОТАТЬ С FLOW:
1. **Создать блок**: двойной клик на канвасе или кнопка "+" внизу
2. **Настроить**: ввести промпт, выбрать модель в панели настроек
3. **Соединить**: навести на блок → появятся точки → перетащить связь к другому блоку
4. **Запустить**: нажать ▶ в блоке для генерации
5. **Сохранить**: кнопка "Сохранить" в правом верхнем углу

### СОВЕТЫ ПО FLOW:
- Начинай с текстового блока — AI сгенерирует хороший промпт
- I2V даёт больше контроля: сначала картинка, потом видео. Формат видео = формат картинки.
- Для вертикальных видео (9:16): используй T2V режим с Kling/Veo и выбери 9:16
- Для keyframe-морфинга: подключи 2 картинки к видео-блоку
- В текстовом блоке можно загрузить медиа для анализа (опиши картинку, создай промпт)
- Референсы доступны для FLUX, Ideogram, Nano Banana
